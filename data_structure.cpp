...
// Global variables, purposely used with the DAC implementation
int vldesc_ln(20), srcvnf_ln(29), dstvnf_ln(29), glk_ln(83), nb_accepted_vnf(0), nb_rejected_vnf(0);
bool accepted;
string output_if_succeeded;
map<int, int> vnf_placed_map;
map<int, bool> used_nodes_map;
map<int, bool> used_edges_map;
vector<int> mypath;
...
void Graph::add_vertices_map(map<int, struct edge_endpoint>& vertices_internal_map_) {
    ...
    // Getting the nodes
    for(auto& v: vertices_internal_map)
    {
        if(get_elt_index(all_vertices,v.first) == -1)
            all_vertices.push_back(v.first);
    }
    nb_vertices = (int) all_vertices.size();
}
...
// We use these functions to retrieve a temporary set of really used edges
void Graph::reset_tmp_used_edges() {
    tmp_used_edges.clear();
}
void Graph::update_tmp_used_edges(int const & i) {
    if(get_elt_index(tmp_used_edges,i) == -1)
        tmp_used_edges.push_back(i);
}
...
// This function returns a specific subset
...
int Graph::find_root(int i) {
	// find root and make root as parent of i (path compression)
	if (subsets[i].parent != i)
    {
		subsets[i].parent = find_root(subsets[i].parent);
    }
...
void Graph::union_subsets(int x, int y) {
	int xroot(Graph::find_root(x));
	int yroot(Graph::find_root(y));
...
bool Graph::union_find_traversal() {
	...
	//subsets = new vector<struct subset>(V);
	...
	for(auto& v : all_vertices)
	{
		//subsets[v].parent = v;
		//subsets[v].rank = 0;
		//subsets->operator[](v) = {v,0};
		subsets.push_back({v,0});
	}

	// Iterate through all edges of graph, find sets of both
	// vertices of every edge, if sets are same, then there is cycle in graph.
    //for(int e(1); e <= E; ++e)
    for(auto& e : edges_id_tab)
	{
		int x = Graph::find_root(edges_internal_map[e].source);
		int y = Graph::find_root(edges_internal_map[e].destination);

		if (x == y) has_cycle = true;

		Graph::union_subsets(x, y);
	}
...
string Graph::get_all_subsets_infos() {
	...
	for (vector<int>::iterator v=all_vertices.begin(); v!=all_vertices.end();++v)
	{
...
void Graph::get_graph_recap() {
    ...
    // Processing
    output += to_string(Graph::get_nb_edges()) + ", Nb Nodes => " + to_string(Graph::get_nb_vertices()) + ", ";
...
// Getting the nodes IDs tab
vector<int> Graph::get_all_vertices() const {
    return all_vertices;
}
...
// Getting the tmp_used_edges vector
vector<int> Graph::get_tmp_used_edges() const {
    return tmp_used_edges;
}
...
void VNC::add_vnc_infos_map(int const & key, string const & given_value) {
    switch(key)
    {
    case 0:
    ...
    for(auto& id : edges_id_tab)
    {
           ...
           || (graph_.get_edge_addr_by_id(id)->source == edge_dst && graph_.get_edge_addr_by_id(id)->destination == edge_src))
        {
            ...
            if(graph_.get_edge_addr_by_id(id)->residual_bandwidth >= requested_bandwidth)
            {
                ...
                graph_.update_tmp_used_edges(id);
                result = true;
                break;
            }
        }
...
// We combine the Divide And Conquer approach with our Union-find adapt.
//This recursive function is the result of our experimentations
void dac_with_vlinks_tab(VNC* myvnc, Graph & mygraph, vector<int> & vlink_id_tab) {
    // Declarations & Initializations
    int node_vnf_src, node_vnf_dst;
    int length_tab = (int) vlink_id_tab.size();

    if(length_tab > 1)
    {
        // Divide Process
        int half_length_tab = length_tab / 2;
        vector<int> vit_part1 = {vlink_id_tab.begin(), vlink_id_tab.begin()+half_length_tab};
        vector<int> vit_part2 = {vlink_id_tab.begin()+half_length_tab, vlink_id_tab.end()};

        // Recursion Process
        dac_with_vlinks_tab(myvnc, mygraph, vit_part1);
        dac_with_vlinks_tab(myvnc, mygraph, vit_part2);


    }
    else //if(length_tab == 1)
    {
        // Solving individual Problem
        int link_id = vlink_id_tab[0];
        struct vnf* vnf_src = myvnc->get_vlink_by_id(link_id)->source_vnf;
        struct vnf* vnf_dst = myvnc->get_vlink_by_id(link_id)->destination_vnf;
        struct vlink_support* vlink_ptr = myvnc->get_vlink_support(link_id);

        string tmp_str1 = " " + myvnc->get_vlink_by_id(link_id)->description + " ";
		...
		if(node_vnf_src == node_vnf_dst)
		{
			...
			if(used_nodes_map.find(node_vnf_src) == used_nodes_map.end())
				used_nodes_map.insert({node_vnf_src, true});

			if(used_nodes_map.find(node_vnf_dst) == used_nodes_map.end())
				used_nodes_map.insert({node_vnf_dst, true});
		...
		else
		{
			...
			for(map<int,vector<int>>::iterator it1=vlink_inside_graph.begin();it1!=vlink_inside_graph.end();++it1)
			{
				...
				if(vlink_supported_by_edge(mygraph, myvnc, link_id, it1->second[0], it1->second[1]))
				{
					...
					if(used_nodes_map.find(real_node_src) == used_nodes_map.end())
						used_nodes_map.insert({real_node_src, true});

					if(used_nodes_map.find(real_node_dst) == used_nodes_map.end())
						used_nodes_map.insert({real_node_dst, true});
				}
				else
				{
					nb_accepted_vnf -= 1;
...
//string vnf_placement_in_graph(Graph & mygraph_, vector<int> & sorted_vertices, map<int, VNC> & vnc_map) {
    ...
    int nb_used_nodes(0), nb_used_edges(0);
    int real_nb_accepted_vnf(0), real_nb_rejected_vnf(0);
    int nb_vnf(myvnc->get_nb_vnf());
    string final_output(""), output(""), final_path, vlink_support_recap, output_if_failed("VNC placement failed!");
    ...
    for(vector<int>::iterator it=sorted_vertices.begin(); it!= sorted_vertices.end(); ++it)
    {
       ...
       for(vector<int>::iterator it0=mypath.begin(); it0!=mypath.end(); ++it0)
       {
        ...
        // We will iterate through the VNC virtual links, using a DAC implementation
        vector<int> vlink_id_tab = myvnc->get_vlink_id_tab();
        dac_with_vlinks_tab(myvnc, mygraph, vlink_id_tab);
        ...
        if(accepted)
        {
            ...
            for(auto& vl : myvnc->get_vlink_process_status())
            {
                if(vl.second.supported)
                {
                    ...
                    struct vnf* vnf_src1 = myvnc->get_vlink_by_id(vl.second.v_id)->source_vnf;
                    struct vnf* vnf_dst1 = myvnc->get_vlink_by_id(vl.second.v_id)->destination_vnf;

                    if(!myvnc->get_singlevnf_placement_status(vnf_src1->vnf_id))
                    {
                        myvnc->update_vnf_placement_status(vnf_src1->vnf_id, true);
                    }
                    if(!myvnc->get_singlevnf_placement_status(vnf_dst1->vnf_id))
                    {
                        myvnc->update_vnf_placement_status(vnf_dst1->vnf_id, true);
                    }
                }
            }
            vlink_support_recap += "]";
            ...
            final_path += '\n';
            output_if_succeeded += "]";
            vector<int> tmp_used_edges = mygraph.get_tmp_used_edges();
            for(vector<int>::iterator it3= tmp_used_edges.begin();it3!=tmp_used_edges.end();++it3)
            {
                used_edges_map.insert({*it3, true});
            }

            //nb_used_nodes = mygraph.get_used_nodes_map_size();
            //nb_used_edges = mygraph.get_used_edges_map_size();
            nb_used_nodes = used_nodes_map.size();
            nb_used_edges = used_edges_map.size();

            //for(auto& it2 : mygraph.get_used_nodes_map())
            for(auto& it2 : used_nodes_map)
            {
            ...
            for(auto& it3 : used_edges_map)
            {
                ...
                long initial_bw((mygraph_.get_edge_addr_by_id(it3.first))->residual_bandwidth);
                int rate_used_bw(0);
...
string requests_placement_process(Graph & mygraph_,// vector<int> & sorted_vertices,
                                  ...
                                  map<int, VNC> & vnc_map//, map<int, struct report_recap> & trequests_recap_map
) {
    ...
    map<int, struct report_recap> trequests_recap_map;

    for(auto& vnc : vnc_map)
    {
        vector<int> sorted_vertices = sort_vertices_by_resources(vertices_map);
        ...
        //final_output += to_string(vnc.first) + " ";
    }
    ...
    //final_output = "\nAccepted requests= " + to_string(nb_accepted_req);
    return final_output;
}
...
void infra_and_requests_mass_loading(map<int, struct edge_struct>& edges_map, map<int, struct edge_endpoint>& vertices_map,
    ...
    /*
        ...
        the nodes and the links inside a network, and the vnf and virtual links of
        several virtual network chain requests
    */
